from neuralop.models import TFNO   # pip install neuraloperator
...
save_dir = 'saved_models/convection_diffusion'
...
# -----------------------------------------------------------------------------
# 1) Build data for PINNs/DeepONet/FNO (pinn_data), PI-BSNet (bsnet_data),
...
# precompute time‐spline for PI-BSNet / PINN
...
# -----------------------------------------------------------------------------
    ...
    pinn_data, bsnet_data, fno_data, ic_data, bc_data = [], [], [], [], []
    ...
    torch.save(bsnet_data, os.path.join(save_dir, 'bsnet_data.pt'))
...
bsnet_path = os.path.join(save_dir, 'bsnet_data.pt')
...
_full_bsnet_data = torch.load(bsnet_path)
...
bsnet_data = _full_bsnet_data[:n_samples]
...
print(f"Loaded {len(pinn_data)} PINN samples, "
      f"{len(bsnet_data)} PI-BSNet samples, "
...
class PIFNO(nn.Module):
    """
    Physics-Informed FNO: applies TFNO on the full 4-channel (λ,a,x,t) grid.
    """
    def __init__(self, modes_x=16, modes_t=16, width=64):
        super().__init__()
        # TFNO(n_modes, in_channels, out_channels, hidden_channels, n_layers)
        self.fno = TFNO(
            n_modes=(modes_x, modes_t),
            in_channels=4,
            out_channels=1,
...
class PIBSNet(nn.Module):
    """PI-BSNet: predict U_pred, then assemble & evaluate in training loop"""
...
model_constructors = {
    ...
    'PIBSNet_FD':     lambda: PIBSNet(n_cp_x, n_cp_t, d, hidden_dim).to(device),
    'PIBSNet':        lambda: PIBSNet(n_cp_x, n_cp_t, d, hidden_dim).to(device),
}
...
for name, ctor in model_constructors.items():
    ...
    summary[name] = {'train_time': elapsed, 'final_loss': total_hist[-1]}
    trained_models[name] = model

    # save checkpoint
    ckpt_path = os.path.join(save_dir, f"{name}_{epochs}epochs.pth")
    torch.save(model.state_dict(), ckpt_path)
    print(f"✔ Saved {name} checkpoint to {ckpt_path}")

    # save histories & time
    np.save(os.path.join(save_dir, f"{name}_{epochs}epochs_total.npy"), total_hist)
    np.save(os.path.join(save_dir, f"{name}_{epochs}epochs_pde.npy"),   pde_hist)
    np.save(os.path.join(save_dir, f"{name}_{epochs}epochs_data.npy"),  data_hist)
    np.save(os.path.join(save_dir, f"{name}_{epochs}epochs_icbc.npy"),  icbc_hist)
    np.save(os.path.join(save_dir, f"{name}_{epochs}epochs_time.npy"),  np.array([elapsed]))

    print(f"{name} done in {elapsed:.1f}s, final loss {total_hist[-1]:.3e}")


# 2) Test parameters and ground truth setup
# By default n_test=1 uses the current (lam_test_default, a_test_default).
# If n_test>1, it samples random (lam_test, a_test) pairs for testing.
n_test = 1  # <-- set >1 to test multiple random combinations
...
if n_test == 1:
    ...
    rng = np.random.default_rng(seed=0)  # optional reproducibility
...
for name, ctor in model_constructors.items():
    ...
    model.eval()
    print(f"\n=== Testing {name} (loaded from {ckpt}) ===")

    # Evaluate over n_test parameter pairs, then average metrics
    L2_list, relL2_list, max_list = [], [], []

    # Only plot/save surfaces for the first test case (to avoid too many PNGs)
    first_case_surfaces = None  # (F_true, B_pred, err, Xg, Tg)

    for case_id, (lam_test, a_test) in enumerate(test_pairs):
        ...
        max_list.append(err.max())

        # Keep first case for plotting
        if case_id == 0:
            first_case_surfaces = (F_true, B_pred, err, Xg, Tg)

    # Average over test pairs
    L2_mean = float(np.mean(L2_list))
    relL2_mean = float(np.mean(relL2_list))
    max_mean = float(np.mean(max_list))

    summary[name].update({
        'L2_error': L2_mean,
        'rel_L2_error': relL2_mean,
        'max_error': max_mean
    })

    print(
        f"  Averaged over n_test={len(test_pairs)} cases: "
        f"L2 error = {L2_mean:.3e}, rel L2 = {relL2_mean:.3e}, max error = {max_mean:.3e}"
    )

    # Plot/save only the first test case
    if first_case_surfaces is not None:
        F_true, B_pred, err, Xg, Tg = first_case_surfaces
        ...
        # Save to <save_dir>/plot_<ModelName>.png
        out_file = os.path.join(save_dir, f"plot_{name}.png")
        fig.savefig(out_file, dpi=300)
        plt.close(fig)


# directory where you saved the .npy histories

# list of model names
names = list(model_constructors.keys())

# epoch axis
epochs_arr = np.arange(1, epochs + 1)

# prepare subplots for PDE, Data, and IC/BC losses
fig, axes = plt.subplots(3, 1, figsize=(10, 12), sharex=True)

for name in names:
    # load each component history
    pde_hist  = np.load(os.path.join(save_dir, f'{name}_{epochs}epochs_pde.npy'))
    data_hist = np.load(os.path.join(save_dir, f'{name}_{epochs}epochs_data.npy'))
    icbc_hist = np.load(os.path.join(save_dir, f'{name}_{epochs}epochs_icbc.npy'))

    # plot on the respective subplot
    axes[0].plot(epochs_arr, pde_hist,  label=name)
    axes[1].plot(epochs_arr, data_hist, label=name)
    axes[2].plot(epochs_arr, icbc_hist, label=name)

# set titles and labels
axes[0].set_title('PDE Loss vs Epoch')
axes[1].set_title('Data Loss vs Epoch')
axes[2].set_title('IC/BC Loss vs Epoch')
for ax in axes:
    ax.set_ylabel('Loss')
    # ax.set_yscale('log')
...
plt.tight_layout()
plt.savefig(os.path.join(save_dir, 'plot_2.png'))
plt.close()
...
for name in model_constructors.keys():
    ...
    # 2) fetch the error you computed earlier (using rel_L2 by default)
    err = summary[name]['rel_L2_error']
...
plt.ylabel('Prediction Relative L2 Error', fontsize=12)
